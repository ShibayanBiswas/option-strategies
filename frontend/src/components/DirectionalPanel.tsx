...
/** Directional risk panel — static UI (no hover scale / enter bounce on click updates). */
export function DirectionalPanel({ staticLatex, staticParagraphs, live }: DirectionalPanelProps) {
  ...
  const needleDeg = live?.label === "net-bearish" ? -35 : live?.label === "net-bullish" ? 35 : 0;

  return (
    <div className="directional-panel space-y-6 w-full">
      {live && bias && (
        <div className="directional-hero directional-hero-full">
          <div className="directional-compass">
            <svg viewBox="0 0 120 120" className="directional-compass-svg">
              ...
              <circle
                ...
                strokeDashoffset={326 - deltaMag * 326}
              />
              <g style={{ transformOrigin: "60px 60px", transform: `rotate(${needleDeg}deg)` }}>
          ...
          <div className="directional-metrics directional-metrics-full">
            <div className="directional-metric directional-metric-delta">
            ...
            <div className="directional-metric directional-metric-slope">
            ...
            <div className="directional-metric directional-metric-gamma">
              <span className="directional-metric-key">
                <Latex math="\Gamma_{\text{net}}" />
              </span>
              <span className="directional-metric-val">{signedFmt(live.netGamma, 3)}</span>
            </div>
            <div className="directional-metric directional-metric-vega">
              <span className="directional-metric-key">
                <Latex math="\nu_{\text{net}}" />
              </span>
              <span className="directional-metric-val">{signedFmt(live.netVega, 3)}</span>
            </div>
            <div className="directional-metric directional-metric-theta">
              <span className="directional-metric-key">
      ...
      {paragraphs && paragraphs.length > 0 && (
        <div className="research-prose directional-prose">
          {capParagraphs(paragraphs).map((p, i) => (
            <p key={i} className="directional-prose-line">
              <ProseMath text={p} />
            </p>
          ))}
        </div>
      ...
      {live?.legContributions && live.legContributions.length > 0 && (
        <div className="directional-legs directional-legs-full">
          ...
          <div className="directional-legs-grid">
            {live.legContributions.map((lc) => (
              <div key={lc.leg} className="directional-leg-card">
