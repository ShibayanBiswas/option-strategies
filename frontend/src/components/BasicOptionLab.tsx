...
export function BasicOptionLab({ options }: BasicOptionLabProps) {
  ...
  return (
    <div className="research-lab w-full overflow-hidden rounded-lg border border-slate-800/60 bg-surface/30 p-6">
      <h3 className="text-lg font-semibold text-white mb-4 font-serif italic">Laboratory Controls</h3>
        ...
        {options.map((opt, i) => (
          <motion.button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all border font-serif ${
              active === i
                ? "bg-gradient-to-r from-cyan-500/20 to-emerald-500/15 border-cyan-500/40 text-white"
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="rounded-xl border border-surface-border bg-surface/40 p-4">
            <p className="text-xs uppercase tracking-wider text-slate-500 mb-3 font-serif">Parameters</p>
            <ParamControls
